...
type HandshakeInfo struct {
	...
	HeadBlockNum             uint32
	HeadBlockID              flon.Checksum256
	HeadBlockTime            time.Time
	LastIrreversibleBlockNum uint32
	LastIrreversibleBlockID  flon.Checksum256
}
...
func (p *Peer) Read() (*flon.Packet, error) {
	packet, err := flon.ReadPacket(p.reader)
...
func (p *Peer) WriteP2PMessage(message flon.P2PMessage) (err error) {
...
func (p *Peer) SendSyncRequest(startBlockNum uint32, endBlockNumber uint32) (err error) {
	...
	syncRequest := &flon.SyncRequestMessage{
...
func (p *Peer) SendRequest(startBlockNum uint32, endBlockNumber uint32) (err error) {
	...
	request := &flon.RequestMessage{
		ReqTrx: flon.OrderedBlockIDs{
			Mode:    [4]byte{0, 0, 0, 0},
			Pending: startBlockNum,
		},
		ReqBlocks: flon.OrderedBlockIDs{
			Mode:    [4]byte{0, 0, 0, 0},
...
func (p *Peer) SendNotice(headBlockNum uint32, libNum uint32, mode byte) error {
	...
	notice := &flon.NoticeMessage{
		KnownTrx: flon.OrderedBlockIDs{
			Mode:    [4]byte{mode, 0, 0, 0},
			Pending: headBlockNum,
		},
		KnownBlocks: flon.OrderedBlockIDs{
			Mode:    [4]byte{mode, 0, 0, 0},
			Pending: libNum,
		},
	}
...
func (p *Peer) SendHandshake(info *HandshakeInfo) error {
	...
	tstamp := flon.Tstamp{Time: info.HeadBlockTime}
	...
	handshake := &flon.HandshakeMessage{
		NetworkVersion:           1206,
