...
func ExampleAPI_PushTransaction_transfer_EOS() {
	...
	from := flon.AccountName("eosuser1")
	to := flon.AccountName("eosuser2")
	quantity, err := flon.NewEOSAssetFromString("1.0000 EOS")
	memo := ""
	...
	signedTx, packedTx, err := api.SignTransaction(context.Background(), tx, txOpts.ChainID, flon.CompressionNone)
	if err != nil {
