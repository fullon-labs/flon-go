...
type GenesisState struct {
	//InitialConfiguration ChainConfig //flon.ChainConfig
	InitialTimestamp flon.TimePoint
	InitialKey       ecc.PublicKey
}

type ChainConfig struct {
	MaxBlockNetUsage               flon.Uint64 ///< the maxiumum net usage in instructions for a block
...
func readGenesisState(section *Section, f sectionCallbackFunc) error {
	...
	// assert.NoError(t, flon.UnmarshalBinary(cnt, &state))
