...
func ExamplePackedTransaction_Pack() {
	...
	transferPermissionLevel, err := flon.NewPermissionLevel(from + "@active")
	NoError(err, "parse permission level")

	transferQuantity, err := flon.NewAssetFromString("10.0000 EOS")
	NoError(err, "parse asset")

	trx := flon.NewTransaction([]*flon.Action{
		{
			Account:       "eosio.token",
			Name:          "transfer",
			Authorization: []flon.PermissionLevel{transferPermissionLevel},
			ActionData: flon.NewActionData(&flon.Transfer{
				From:     flon.AccountName(from),
				To:       "eosio.token",
	...
	signedTrx, err := keyBag.Sign(context.Background(), flon.NewSignedTransaction(trx), mainnetChainID, signerKey)
	NoError(err, "sign transaction")

	packedTrx, err := signedTrx.Pack(flon.CompressionNone)
...
func encode(v interface{}) string {
	...
	err := flon.NewEncoder(buffer).Encode(v)
	NoError(err, "encode %T", v)

	return hex.EncodeToString(buffer.Bytes())
}

func decodeChainID(in string) flon.Checksum256 {
