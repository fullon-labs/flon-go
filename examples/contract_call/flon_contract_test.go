...
func TestQueryBuyerBids(t *testing.T) {
	...
	params := flon.GetTableRowsRequest{
		Code:  "nftone.mart",
		Scope: "nftone.mart",
		Table: "buyerbids",
	...
	var tableRows *flon.GetTableRowsResp
	// test
	for i := 0; i < 4; i++ {
...
func getQuery(name string, orderId uint64) (string, string, error) {
	actual, err := flon.ExtendedStringToName(name)
	if err != nil {
...
func TestQuerySellOrders(t *testing.T) {
	...
	maker := "yanjingxun33"
	actual, err := flon.ExtendedStringToName(maker)
	if err != nil {
		fmt.Printf("string to name error: %s", err.Error())
		return
	}
...
func retryQuery(ctx context.Context, num int, client *flon.API, params flon.GetTableRowsRequest) (*flon.GetTableRowsResp, error) {
	var tableRows *flon.GetTableRowsResp
	var err error
	for i := 0; i < num; i++ {
