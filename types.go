...
// NewAssetFromString reads a string and decodes it to an flon.Asset
...
func (a *BaseVariant) UnmarshalJSON(data []byte, def *VariantDefinition) error {
	...
	if typeGo.Kind() == reflect.Ptr {
		...
		// the equivalence of doing `name := flon.Name("")`) and take the
...
//
// **Note** For `Int64` and `Uint64`, we return `flon.Int64` and `flon.Uint64` types
//
